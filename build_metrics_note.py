...
def main():
    ...
    D.tbl("2 つの決定係数",
          ...
          "（分母が計算値の分散）を用いている。</b>")
    D.a('<p class="note bad"><b>訂正。</b>初版はこの形を「AIJ ガイドブック '
        "5.2 と同じ形」と書いていた。<b>確かめていない。</b>"
        "資料から式を転記して照合しておらず、そう書ける根拠が無い。"
        "指摘を受けて撤回する。</p>"
        "<p><b>一般には、決定係数の分母（全変動）は実測値の側に取ることが"
        "多い。</b>その意味でも、上段の形が資料と一致しているかは"
        "確かめる必要がある。<b>確かめるまでは「本サイトはこの定義を"
        "使っている」とだけ書き、資料の側の定義には触れない。</b>"
        "どちらの定義でも値を出せるよう、次の表に両方を並べてある。</p>"
        '<p class="capnote"><b>未解決。</b>ガイドブック 5.2 の式を'
        "照合すること。照合するまで、本サイトのどの報告書でも"
        "「ガイドブックと同じ定義である」とは書かない。</p>")
    rr2 = []
