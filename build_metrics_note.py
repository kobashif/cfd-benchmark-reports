...
def main():
    ...
    ap.add_argument("--wd", nargs="+", default=None,
                    help="載せる風向。**公開済みのものに限ること。**"
                         "省略すると hitrate にあるもの全部")
    a = ap.parse_args()
    date = a.date or datetime.date.today().isoformat()
    HR = jload(a.hitrate)
    # **公開していない風向を例に出さない。**読み手はその風向の中身を
    # どこにも確かめに行けない。指標の話をする頁で、確かめられない数を
    # 並べるのは筋が通らない。"_pooled" のような集計の鍵も外す。
    if HR:
        keep = set(a.wd) if a.wd else None
    ...
    D.cover()
    # **履歴は積み上げる。**版を渡すたびに初版の行を書き換えていたので、
    # 版 1.1 を出すと「1.1 初版」という行になり、1.0 が消えていた。
    # 版を足すときはここに 1 行足す。**行を書き換えない。**
    REVS = [
        ["1.0", "2026-09-06",
         "初版。hit rate の導入、W の決め方、"
         "測点を除いてよい条件、ケース F 東風向の実例"],
        ["1.1", "2026-09-07",
         "決定係数の節を、採用した定義を述べる形に改めた。"
         "他の報告と並べるときの注意（傾きは直接比較できる、"
         "R² は定義を確かめる）を残し、"
         "どの資料に定義が載っているかの議論は落とした。"
         "定義の比較表は、公開済みの風向だけを載せるようにした。"],
    ]
    if a.rev not in [r[0] for r in REVS]:
        raise SystemExit("**版 %s が改訂履歴に無い**" % a.rev)
    D.revisions([r for r in REVS
                 if [x[0] for x in REVS].index(r[0]) <=
                 [x[0] for x in REVS].index(a.rev)])
    D.toc_placeholder()
    ...
    D.tbl("2 つの決定係数",
          ...
          "（分母が計算値の分散）を用いている。</b>")
    D.a('<p class="note"><b>本サイトは上段（原点通過回帰・分母は計算値の'
        "分散）を用いる。</b>"
        "<b>傾き a は定義が一意（a = &Sigma;xy/&Sigma;x²）なので、"
        "他の報告と直接比べてよい。</b>"
        "R² は定義によって値も符号も変わるので、"
        "<b>他の報告の R² と並べるときは、相手がどちらの定義かを"
        "確かめること。</b>"
        "定義を書かずに R² だけを並べた比較は、"
        "同じ量を比べているとは限らない。</p>")
    rr2 = []
    ...
    # -------------------------------------------------- 収束条件 --
    D.h2("5", "収束条件をどう決めるか")
    D.a("<p><b>収束条件に一般的な基準は無い。</b>これは当方の言い分では"
        "なく、AIJ ガイドブック（第 2 編 9「収束条件」）がそう書いている。"
        "同節は圧力のポアソン方程式の打ち切り条件を 6 通り挙げたうえで、"
        "「汎用のソフトウェアでは &epsilon;&prime; = 10<sup>&minus;4</sup> 程度が"
        "デフォルト値となっている事が多いようである」と述べるにとどめている。"
        "<b>本サイトが <code>residualControl</code> の値を「当方の判断」と"
        "書いてきたのは、この意味である。</b></p>")
    D.a('<div class="note bad"><h4>同節が求めている確認を、まだしていない</h4>'
        "<p>同節はこう続ける&mdash;<b>「定常計算においては、収束条件が"
        "最終的な定常場の速度分布に大きな影響を及ぼすことから、2 桁程度、"
        "条件を厳しくして計算結果に変化がないか確認するのが望ましい」。</b>"
        "厳しくない場合は<b>「速度の空間変化が過度に緩和され、強風域、"
        "弱風域が適切に予測されない可能性がある」</b>とも書いている。</p>"
        "<p><b>この確認を、正面からやったケースはまだ無い。</b>"
        "近いことはしている&mdash;ケース C は 5,000 から 13,000 反復まで"
        "延ばして評価高さの変化を測り、ケース A は打ち切ったモデルを"
        "回し直した。<b>ただしどちらも「反復を伸ばす」向きの確認であって、"
        "「収束条件を厳しくする」確認ではない。</b></p>"
        "<p>次にやることとして残す&mdash;代表ケース 1 つで収束条件を "
        "2 桁厳しくして回し、<b>報告する量が変わらないことを確かめる。</b>"
        "変わるなら厳しい側を採る。</p></div>")
    ...
    D.a("<ol>"
        ...
        "<li>日本建築学会 編著（2020）『都市の風環境予測のための CFD "
        "ガイドブック』丸善出版、ISBN 978-4-8189-2718-6。"
        "第 2 編 9「収束条件」p.106（本頁 5 章）、"
        "第 3 編 5.2 pp.148-152（本頁 1 章）</li>"
